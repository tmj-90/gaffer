...
| **Runner** | `runner/` | Bash orchestrator — spawns a `claude -p` agent per ticket, 31-skill library, deterministic safety hook, git-worktree isolation, model tiering. |
...
Create `runner/skills/<name>/SKILL.md`. The runner's 31-skill library is loaded from this directory. Each skill is a structured markdown prompt fragment that the agent receives for specific task types. No TypeScript compilation needed.
