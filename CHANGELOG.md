...
- **Generated configuration reference.** `docs/CONFIG.md` is derived from the code by `scripts/config-reference.mjs`: every `factory.config.sh` default (grouped by the file's own section banners, with mode-branch fills distinguished from the real default), whether the dashboard can edit it, which components read it, the Settings-panel-only keys, and every factory-prefixed variable the code reads without ever assigning — the list of knobs that have no default or UI entry. CI fails when the committed reference is stale (`--check`).
