...
| **Runner** · `runner/` | The orchestrator — bash that spawns a `claude -p` agent per ticket, with a 37-skill library, a deterministic safety hook, git-worktree isolation, and model tiering (plan on a strong model, implement on a fast one). |
...
├── runner/           bash orchestrator, 37-skill library, safety hook
├── pnpm-workspace.yaml
└── package.json
```
...
- Runner factory loop with 37-skill library and model tiering
...
- No third-party skill *marketplace* yet. The 37 bundled skills load automatically — the factory injects the whole library into every ticket's agent, and `gaffer skills install` adds them to your own Claude Code. Authoring a new skill is still just dropping a `SKILL.md` into `runner/skills/`.
