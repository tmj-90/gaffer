...
- **Notification sinks are an outbound channel.** The webhook (`GAFFER_NOTIFY_WEBHOOK_URL`) and Slack (`GAFFER_NOTIFY_SLACK_URL`) sinks POST human-gate events — including ticket metadata — to a URL you configure. The full event body carries the ticket `title` and `detail` free text, which a prompt-injected agent can influence, so an untrusted endpoint receiving these is an exfiltration path. Set `GAFFER_NOTIFY_REDACT=1` to send a **minimal** body (kind · ticket number · status · dashboard URL only), dropping the agent-influenceable free text. Treat the notify URL like any other outbound integration — point it only at endpoints you trust.
