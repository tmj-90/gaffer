...
describe("parseFeatureNames", () => {
  it("extracts feature names from a human `features` listing", () => {
    const stdout = [
      "Features for betbridge: 3",
      "",
      "SHIPPED (3)",
      "  [shipped] Bet slip  (ab12cd34)",
      "  [shipped] Odds feed  @Pricing  (ef56gh78)",
      "      a summary line that must be ignored",
      "  [shipped] Settlement engine  (Core)  (ij90kl12)",
    ].join("\n");

    const names = parseFeatureNames(stdout, "betbridge").map((f) => f.name);
    expect(names).toEqual(["Bet slip", "Odds feed", "Settlement engine"]);
  });

  it("returns [] for the empty listing", () => {
    expect(parseFeatureNames("memory: no features for 'betbridge'", "betbridge")).toEqual([]);
  });
});
...
describe("CliMemoryClient write verbs", () => {
  it("builds the digest set argv with every section + source, defaulting a null stack", async () => {
    ...
    expect(result).toEqual({ repo: "betbridge", status: "updated" });
  ...
  it("builds the feature add argv as a repo positional with optional scope-node", async () => {
    ...
    await client.addFeature(feature("Bet slip"));
    await client.addFeature(feature("Odds feed", "Pricing"));
  ...
  it("parses listFeatures output and surfaces a non-zero exit as unavailable", async () => {
    const okClient = new CliMemoryClient({
      ...
      runner: recordingRunner({
        features: { status: 0, stdout: "  [shipped] Bet slip  (ab12cd34)\n", stderr: "" },
      }).runner,
    });
    expect((await okClient.listFeatures("betbridge")).map((f) => f.name)).toEqual(["Bet slip"]);
    ...
    await expect(badClient.listFeatures("betbridge")).rejects.toThrow(/MEMORY_UNAVAILABLE|error/i);
  });
...
describe("flushRepoUnderstanding through the CLI client de-dupes on re-onboard", () => {
  it("writes the digest + every feature first, then skips already-recorded features", async () => {
    ...
    const understanding = {
      digest,
      features: [feature("Bet slip"), feature("Odds feed", "Pricing")],
    };
    ...
    expect(stored).toEqual(["Bet slip", "Odds feed"]);
    ...
    expect(stored).toEqual(["Bet slip", "Odds feed"]); // unchanged — no duplicate adds
  });
});
