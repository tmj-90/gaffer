...
describe("Scope graph: data layer (FG-001 + FG-002)", () => {
  describe("scope nodes", () => {
    it("creates, lists, gets, renames and deletes a node", () => {
      ...
      const node = wg.createScopeNode(
        { name: "Marketplace", type: "product", owner: "trading", tags: ["commerce"] },
        human,
      );
      expect(node.id).toBeTruthy();
      expect(node.type).toBe("product");
      expect(node.tags_json).toBe(JSON.stringify(["commerce"]));

      expect(wg.listScopeNodes()).toHaveLength(1);

      const view = wg.getScopeNode(node.id);
      expect(view.node.name).toBe("Marketplace");
      expect(view.repos).toEqual([]);

      const renamed = wg.updateScopeNode(node.id, { name: "Marketplace v2" }, human);
      expect(renamed.name).toBe("Marketplace v2");
  ...
  describe("scope edges", () => {
    it("creates a contains edge and lists it (filtered by node)", () => {
      const wg = fresh();
      const parent = wg.createScopeNode({ name: "Marketplace", type: "product" }, human);
