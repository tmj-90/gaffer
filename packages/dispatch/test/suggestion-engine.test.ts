 ...
 * sportsbook/trading factory graph: a product scope owning its API repo, a
...
describe("FG-005: scope→repo suggestion engine", () => {
  it("one-to-one: a scope owning a repo suggests WRITE with high confidence", () => {
    ...
    const repo = wg.registerRepository({ name: "sportsbook-api" }, human);
    wg.linkScopeRepo(
    ...
    expect(s.reasons.join(" ")).toContain("scope 'Sportsbook' owns/targets this repo");
    wg.db.close();
  });
