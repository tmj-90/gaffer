...
function mavenRepo() {
  ...
  writeFileSync(
    join(dir, "README.md"),
    "# Demo\n\nA Kafka-native sports data normalisation platform. It connects to suppliers and " +
      "publishes a single canonical feed for sportsbook operators.\n",
  );
...
console.log("== AC5: extract + validate a strict-JSON analysis ==");
{
  const text =
    ...
    JSON.stringify({
      digest: {
        overview: "A sports data normalisation platform.",
...
console.log("== AC18: analyzeAndWrite end-to-end drafts grounded+cited+induction lore ==");
{
    ...
    JSON.stringify({
      ...
      lore: [
        // grounded by a URL → kept, --source set, confidence medium (clamped from high)
        {
          ...
          body: "ADR.",
          tags: ["migrations"],
          source: "https://github.com/acme/betbridge/pull/9",
          confidence: "high",
        },
  ...
  assert(
    ...
    urlDraft.includes("--source") && urlDraft.includes("https://github.com/acme/betbridge/pull/9"),
  );
  assert(
